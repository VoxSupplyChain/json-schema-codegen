...
abstract class ModelGenerator[N: Numeric](
    json2predef: Map[SimpleType.SimpleType, PredefType],
    format2predef: Map[(PredefType, String), PredefType]
) extends Naming {
  ...
  def `object`(schema: Schema, name: Option[String]): SValidation[LangType] =
    schema.obj.toRightDisjunction(s"not object type: ${schema.types}").flatMap { obj =>
      ...
      val propertyTypes: List[SValidation[LangTypeProperty]] = obj.properties.value.map {
        case (propName, propDefinition) =>
          ...
          val propDef = existingType orElse any(propDefinition.schema, propName.some) map { t =>
      ...
      for {
        ...
        additional <-
          obj.additionalProperties.toList
            .map(nested => any(nested, (className + "Additional").some))
            .sequence
            .map(_.headOption)
      } yield {
  ...
  def ref(schema: Schema): SValidation[LangType] =
    for {
      ...
      packageName(ref.id.getOrElse(ref.scope)),
      className(ref.id.getOrElse(ref.scope)),
      List.empty,
      Option.empty
    )
  ...
  def array(schema: Schema, name: Option[String]): SValidation[LangType] =
    for {
      ...
      arrayschema <-
        array.items.value.headOption.toRightDisjunction(s"items ${name.getOrElse("")} is empty: ${schema.types}")
  ...
  def simple(schema: Schema, name: Option[String]): SValidation[LangType] =
    if (schema.enums.isEmpty)
      for {
        ...
        val formatType =
          schema.format.flatMap(format => format2predef.get((simpleType, format.toLowerCase))).getOrElse(simpleType)
        val finalType =
          if (typeName.isEmpty || schema.id.isEmpty) formatType
          else AliasType(packageName(schema.id.getOrElse(schema.scope)), typeName, formatType)
  ...
  def any(schema: Schema, name: Option[String]): SValidation[LangType] =
    if (schema.types.size != 1)
      ...
      enum(schema, name) orElse array(schema, name) orElse `object`(schema, name) orElse simple(
        schema,
        name
      ) orElse -\/(s"Unclear definition: ${name.orEmpty}")
